...
# 1. Insert any missing customers that exist in transaction_data
cur.execute("""
INSERT INTO customers (phone_number, name)
SELECT DISTINCT sender_number, sender_name 
FROM transaction_data 
WHERE sender_number IS NOT NULL 
AND sender_number NOT IN (SELECT phone_number FROM customers)
""")
conn.commit()
